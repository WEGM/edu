...
H.index = {
    ...
    event: function() {
        ...
        $('.switch').click(function(e) {
            e.preventDefault();
            location.href = location.href.replace('review-ac7.html', 'review-ac.html');
        });
    },
    ...
    fillSContent: function(data) {
        if (data.sitem.length > 0) {
            ...
            if (playList.split('~')[0] == 0) {
                ...
                var mp3 = playList.split('~')[1].split(',');
                for (var i = 0; i < mp3.length; i++) {
                    if (data.stuimg) {
                        $('.content').append('<p class="voice stu-voice" data-list="1~' + mp3[i] + '"><span class="avatar-box"><img src="' + data.stuimg + '"></span><i class="icon-voice"></i><em></em><audio src="' + mp3[i] + '" id="mp3vip_' + (i + 1) + ' preload"></audio></p>');
                    } else {
                        $('.content').append('<p class="voice stu-voice" data-list="1~' + mp3[i] + '"><span class="avatar-box" style="background:' + ramdomColor + ';text-align:center;"><i class="key-name">' + data.stuna.substring(0, 1) + '</i></span><i class="icon-voice"></i><em></em><audio src="' + mp3[i] + '" id="mp3vip_' + (i + 1) + ' preload"></audio></p>');
                    }
                };
            }
        }
        ...
        try {
            ...
            audio.onloadedmetadata = function() {
                ...
                if (i < me.mp3List.length) {
                    ...
                    showTips('语音下载完毕，请<span style="color:yellow;"> 点击 </span>播放', null, 1.5e3);
                    me.playDom.attr('mp3load', 'true');
                }
            };
        } catch(e) {
            hideLoading();
