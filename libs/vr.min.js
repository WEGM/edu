...
function pano2vrPlayer(h) {
	...
	function Da() {
		...
		if (0 < y.length)
			for (d = 0; d < y.length; d++) y[d].Ra && y[d].$a != y[d].obj.currentTime && (y[d].$a = y[d].obj.currentTime, !y[d].Va && 0 < y[d].obj.videoHeight && (y[d].Va = y[d].obj.videoWidth / y[d].obj.videoHeight));
		if (0 < R) {
			if (2 == R)
	...
	function Oc() {
		for (var a = new P(0, 0, -100), d = b.S(), c = 0; c < J.length; c++) {
			...
			if ("poly" == f.type) {
				...
				k = Tb(k);
				if (0 < k.length)
					for (h = 0; h < k.length; h++) a = k[h], 0.1 > a.e ? (g = -d / a.e, e = z / 2 + a.x * g, g = t / 2 + a.y * g) : g = e = 0, a.$ = e, a.G = g;
				f.ua = k
			}
		}
	}
	...
	b.updatePanorama = function() {
		...
		if (ga)
				...
				if (g) {
					...
					for (h = D.length - 1; h >= e;) {
						...
						for (curCfNr = 0; 6 > curCfNr; curCfNr++)
							if (f = b.f.j[curCfNr], d = f.ta, ua(ea), Ib(ea, -la * Math.PI / 180, [0, 0, 1]), Ib(ea, -x * Math.PI / 180, [1, 0, 0]), Ib(ea, (180 - u) * Math.PI / 180, [0, 1, 0]), 4 > curCfNr ? Ib(ea, -Math.PI / 2 * curCfNr, [0, 1, 0]) : Ib(ea, Math.PI / 2 * (5 == curCfNr ? 1 : -1), [1, 0, 0]), f.F && d && 0 < d.Ga && 0 < d.Ka && 0 < d.scale || level.t) {
								f.dirty = r;
								level.t ? (c = {
									U: 0,
									V: 0
			...
			if (0 < D.length) {
				...
				b.f.za(u, x);
				for (n = 0; 6 > n; n++)
					if (h = b.f.j[n]) a = "", ma ? (a += "translate3d(" + z / 2 + "px," + t / 2 + "px,0px) ", a += "perspective(" + e + "px) ", a += "translate3d(0px,0px," + e + "px) ") : a += "translate3d(" + z / 2 + "px," + t / 2 + "px," + e + "px) ", a += "rotateZ(" + Number(la).toFixed(10) + "deg) rotateX(" + Number(x).toFixed(10) + "deg)  rotateY(" + Number(-u).toFixed(10) + "deg) ", h.Pa && (a += h.Pa, h.F || (a = "translate3d(-10px,-10px,0px) scale(0.001,0.001)"), h.style[T] = a)
			}
		else if (yb) {
			Ua();
	...
	b.openNext = function(a, d) {
		...
		K();
		if ("{" == a.charAt(0))
			if (f = a.substr(1, a.length - 2), ob[f]) b.Ca(ob[f], c), p(a);
			else {
...
function pano2vrSkin(player, base) {
	...
	this.addSkin = function() {
		...
		this._up__img['ondragstart'] = function() {
			return false
		};
		...
		this._down__img['ondragstart'] = function() {
			return false
		};
		...
		this._loadingtext.ggUpdateText = function() {
			var hs = "\u8d44\u6e90\u6b63\u5728\u52a0\u8f7d " + (me.player.getPercentLoaded() * 100.0).toFixed(0) + "%";
			if (hs != this.ggText) {
				this.ggText = hs;
		...
		this.findElements = function(id, regex) {
			...
			this.__div.onclick = function() {
				if (getQueryString('kfrom')) {
					var kfrom = (me.hotspot.url.indexOf('?') >= 0 ? '&' : '?') + 'kfrom=' + getQueryString('kfrom')
				} else {
					var kfrom = ''
				}
				me.player.openUrl((getQueryString('kfrom') + '-' + me.hotspot.url + kfrom), me.hotspot.target);
			...
			this._hsimage__img['ondragstart'] = function() {
				return false
			};
